...
describe('/api/agents — routes + audit (token-auth era)', () => {
  ...
  it('GET /api/agents stamps each row with ownerName from the owner credential', async () => {
    kit = await startTestServer('agents-list-owner-name');
    // Two distinct admin-issued creds with distinct ownerName values.
    const issueA = await call(kit.baseUrl, 'POST', '/admin/credentials/issue', kit.adminToken, {
      botName: 'alpha-cred', ownerName: 'alpha-team', role: 'member',
    });
    const issueB = await call(kit.baseUrl, 'POST', '/admin/credentials/issue', kit.adminToken, {
      botName: 'beta-cred', ownerName: 'beta-team', role: 'member',
    });
    expect(issueA.status).toBe(201);
    expect(issueB.status).toBe(201);
    const tokenA = issueA.body.token as string;
    const tokenB = issueB.body.token as string;
    await call(kit.baseUrl, 'POST', '/api/agents', tokenA, { botName: 'agent-a1', url: 'http://a1' });
    await call(kit.baseUrl, 'POST', '/api/agents', tokenB, { botName: 'agent-b1', url: 'http://b1' });
    const list = await call(kit.baseUrl, 'GET', '/api/agents', tokenA);
    expect(list.status).toBe(200);
    const byName = new Map<string, string>(
      (list.body.agents as Array<{ botName: string; ownerName: string }>).map(
        (a) => [a.botName, a.ownerName],
      ),
    );
    expect(byName.get('agent-a1')).toBe('alpha-team');
    expect(byName.get('agent-b1')).toBe('beta-team');
  });

  it('GET /api/agents — one credential owning many agents → all rows share the same ownerName', async () => {
    kit = await startTestServer('agents-list-owner-name-shared');
    const issue = await call(kit.baseUrl, 'POST', '/admin/credentials/issue', kit.adminToken, {
      botName: 'bridge-cred', ownerName: 'shared-bridge', role: 'member',
    });
    const token = issue.body.token as string;
    await call(kit.baseUrl, 'POST', '/api/agents', token, { botName: 'bridge-bot-1', url: 'http://1' });
    await call(kit.baseUrl, 'POST', '/api/agents', token, { botName: 'bridge-bot-2', url: 'http://2' });
    const list = await call(kit.baseUrl, 'GET', '/api/agents', token);
    expect(list.status).toBe(200);
    const owners = (list.body.agents as Array<{ ownerName: string }>).map((a) => a.ownerName);
    expect(owners).toEqual(['shared-bridge', 'shared-bridge']);
  });

  it('GET /api/agents — agent owned by a revoked cred is still listed with its prior ownerName', async () => {
    // Revoke is a soft-delete (sets revoked_at; row remains). The owner lookup
    // still succeeds and shows the prior owner — useful for forensics. The
    // `ownerName === ""` empty-string sentinel only fires when the cred row is
    // hard-deleted from the DB, which is not exercised by any production path.
    kit = await startTestServer('agents-list-owner-revoked');
    const issue = await call(kit.baseUrl, 'POST', '/admin/credentials/issue', kit.adminToken, {
      botName: 'short-lived-cred', ownerName: 'short-lived-owner', role: 'member',
    });
    const credentialId = issue.body.credential.id as string;
    const token = issue.body.token as string;
    await call(kit.baseUrl, 'POST', '/api/agents', token, { botName: 'orphaned-bot', url: 'http://o' });
    const revoke = await call(kit.baseUrl, 'POST', '/admin/credentials/revoke', kit.adminToken, {
      credentialId,
    });
    expect(revoke.status).toBe(200);
    // List via admin token (revoked cred can no longer auth).
    const list = await call(kit.baseUrl, 'GET', '/api/agents', kit.adminToken);
    expect(list.status).toBe(200);
    const orphaned = (list.body.agents as Array<{ botName: string; ownerName: string }>)
      .find((a) => a.botName === 'orphaned-bot');
    expect(orphaned).toBeDefined();
    expect(orphaned!.ownerName).toBe('short-lived-owner');
  });
