...
| `METABOT_CORE_URL` | `https://metabot.xvirobotics.com/core` | 中心 metabot-core 服务地址（MetaMemory + Skill Hub） |
| `METABOT_CORE_TOKEN` | 读 `~/.metabot-core/token` | metabot-core Bearer Token |
...
安装器将 `metabot`、`mb` 放到 `~/.local/bin/`，安装后立即可用。`mm`（MetaMemory CLI）和 `mh`（Skill Hub CLI）由 [metabot-core](https://gitlab.xvirobotics.com/xvirobotics/metabot-core) 单独安装。

```bash
# MetaBot 管理（保留子命令，直接由 bin/metabot 处理）
metabot update                      # 拉取最新代码，重新构建，更新 skills，重启
metabot start / stop / restart      # PM2 管理
metabot logs                        # 查看实时日志
metabot status                      # PM2 进程状态

# 功能子命令（其余一律转发给 metabot-core 功能 CLI）
metabot t5t board                   # 团队日报看板
metabot agents list                 # 对端 Bot 通讯录
metabot memory search "部署指南"     # 共享记忆全文搜索
metabot skills list                 # 技能仓库
# 覆盖 metabot-core CLI 路径：export METABOT_CORE_CLI=/path/to/packages/cli/bin/metabot

# MetaMemory（由 metabot-core 提供 mm/mh CLI）
mm search "部署指南"                 # 全文搜索
mm list                             # 列出文档
mm folders                          # 文件夹树
...
CLI 支持连接远程 MetaBot 服务器，在 `~/.metabot/.env` 配置 `METABOT_URL` 即可；MetaMemory CLI 配置见 metabot-core README。
