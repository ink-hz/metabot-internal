...
| **API Key 兜底** | ✅ `ANTHROPIC_API_KEY` / 第三方 Anthropic 兼容端 | ✅ Moonshot API Key | ✅ `CODEX_API_KEY` / `OPENAI_API_KEY` + `CODEX_BASE_URL` |
...
Codex 支持通过本机 `codex exec --json` CLI 接入，并使用 `codex exec resume` 续接聊天会话。启动 MetaBot 前，请先执行 `codex login`，或在 `.env` 里配置 `CODEX_API_KEY` / `CODEX_BASE_URL`（也可在单个 bot 的 `codex.apiKey` / `codex.baseUrl` 覆盖）。MetaBot 会把 API key 标准化成 Codex 子进程的 `OPENAI_API_KEY`，并把 base URL 传成 `-c openai_base_url="..."`。飞书侧的 `/<skill-name> ...` 调用会统一转成 Codex 的 `$<skill-name> ...` 显式技能调用（例如安装了 `/metaschedule` 后，Codex 会收到 `$metaschedule ...`）。
...
└── docs/                      # 全量文档（含吸收过来的 docs/metabot-core/）
```
...
> **模型切换**：每个会话可独立设置模型，默认 `claude-opus-4-8`。所有模型默认 200k 上下文；在模型名后加 `[1m]` 可启用 1M 上下文窗口（Opus 4.8/4.7/4.6、Sonnet 4.6 支持），例如 `/model claude-opus-4-8[1m]`。OAuth/Pro-Max 登录用户 SDK 会丢弃 beta flag，`[1m]` 后缀是唯一可靠的 1M 开启方式。注意：1M 在上下文 <200K 时与 200k 同价，>200K 后全部 token 按 2× 计费，所以默认保持 200k。
